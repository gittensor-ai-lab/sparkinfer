...
#
#   0 * * * * GH_TOKEN="$(gh auth token -u <bot account>)" /path/to/sparkinfer/eval/run_museglimmer_cron.sh >> /tmp/sparkinfer_museglimmer_bot.log 2>&1
#
#   Without GH_TOKEN every tick is refused (cron_common.sh). The bot itself runs origin/main from
#   its own worktree; this script is /path/to/sparkinfer's copy, so keep that checkout on main.
...
#   the lock every tick — flock's bounded wait (cron_common.sh) is meant for short overlaps, not that.
#
...
#   • GPU up → full speed + accuracy eval with the cross-model guards; GPU down → --labels-only.
#   • Auto-merge follows SPARKINFER_MUSEGLIMMER_AUTOMERGE (=1 in .env.eval); unlike the DFlash
#     cron wrapper, this script never forces it.
...
open_lock museglimmer || exit 1
flock -w "$LOCK_WAIT_S" 9 || { note_lock_skip museglimmer; exit 0; }
note_lock_taken museglimmer
cd "$REPO_DIR" || { note_refused museglimmer "cannot enter $REPO_DIR"; exit 1; }
...
require_bot_token museglimmer || exit 1
# The bot runs exactly origin/main, from its own worktree -- not whatever REPO_DIR has checked out.
prepare_bot_tree || { note_refused museglimmer "$TREE_WHY"; exit 1; }
cd "$BOT_TREE" || { note_refused museglimmer "cannot enter $BOT_TREE"; exit 1; }
note_ran museglimmer
...
if gpu_ready; then
  ...
  run_bot museglimmer python3 eval/pr_museglimmer_bot.py "${BOT_ARGS[@]}"
else
  DOWN_N="$(note_gpu_down "$DOWN_FILE" MUSE | tail -1)"
  echo "[$TS] sparkinfer Muse Glimmer bot — pinned GPU $GPU_LABEL down (tick $DOWN_N) — labels only"
  run_bot museglimmer python3 eval/pr_museglimmer_bot.py "${BOT_ARGS[@]}" --labels-only
fi
