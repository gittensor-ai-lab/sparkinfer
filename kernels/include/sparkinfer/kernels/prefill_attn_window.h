...
// Muse Glimmer's register-resident prefill attention (hd128, GQA 16, n_kv_heads 2, int8 KV): the
// first tier launch_prefill_attn_mma_muse_hd128 tries. Returns false if it declines.
bool launch_prefill_attn_fa_muse(
    const void* q, const signed char* k_pool, const signed char* v_pool,
    const void* k_scale, const void* v_scale, const int* block_table, void* attn,
    int n_tokens, int n_q_heads, int n_kv_heads, int head_dim,
    int block_size, float scale, int win_blocks, cudaStream_t stream = nullptr, int q_pos0 = 0);
