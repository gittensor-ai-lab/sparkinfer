...
__global__ void quant_h_q8_1_kernel(const float* __restrict__ h,
                                    si_block_q8_1* __restrict__ y, int n_blocks, int pdl, int gu_sync) {
    if (gu_sync) si_pdl_sync();   // PDL: wait for gate_up's h_scratch writes
    if (pdl) si_pdl_lc();         // PDL: let the dependent down kernel begin its grid spin-up
...
static inline void launch_quant_h(
    const float* h, si_block_q8_1* y, int n_blocks, cudaStream_t stream, int pdl, int gu_sync
) {
    const int qthreads = 256;
    const dim3 grid((n_blocks + (qthreads >> 5) - 1) / (qthreads >> 5));
    if (pdl)
        launch_mmvq_down_kernel(pdl, grid, dim3(qthreads), stream, quant_h_q8_1_kernel, h, y, n_blocks, pdl, gu_sync);
    else
        quant_h_q8_1_kernel<<<grid, dim3(qthreads), 0, stream>>>(h, y, n_blocks, 0, 0);
}
...
void launch_moe_expert_ffn_q4k(
    ...
    const bool gu_trig = gu_pdl_enabled() && mmvq && gu2 && gate_type == 12 && up_type == 12;
    ...
    if (down_mmvq && down_type == 14) {   // 14 = ggml Q6_K
        ...
        const int pdl = gu_trig || down_pdl;
        const int gu_sync = gu_trig ? 1 : 0;
        launch_quant_h(h_scratch, hq8, nqb, stream, pdl, gu_sync);
