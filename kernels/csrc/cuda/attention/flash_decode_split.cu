...
// dynamic smem against the 100 KB an SM has caps residency at 3 blocks, which leaves ~11 KB of the
// unified L1 per block against a 32 KB per-group K working set, so the neighbouring half-sector is
