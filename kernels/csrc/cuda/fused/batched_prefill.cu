...
__global__ void df_gdn_conv_checkpoint_kernel(
    ...
    for (int tok = 0; tok < n_tokens; tok++) {
        ...
        if (do_norm) {
            ...
            __syncthreads();
            cval *= sw[0];
            // WAR guard (same as pf_gdn_conv_kernel / pf_gdn_conv_tile_kernel): the
            // next tok's `sw[t >> 5] = ss` store is unordered against lagging warps'
            // read of this tok's sw[0] without a trailing barrier — a warp that loses
            // the race normalizes with the NEXT token's partial sum.
            __syncthreads();
        }
        out[(size_t)tok * out_dim + hh * head_dim + t] = __float2bfloat16(cval);
        if constexpr (WRITE_CHECKPOINT) {
