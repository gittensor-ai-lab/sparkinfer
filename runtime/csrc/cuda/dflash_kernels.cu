...
void launch_attn_gqa(const void* q, const void* k, const void* v, void* out,
    ...
    if (d == 128) {
        ...
        if (fa_m && fa_l && fa_acc && attn_gqa_split_path_ok(q_len, kv_len, n_q, n_kv, d)) {
            ...
            // Swept end to end at ctx 4k, one binary, draft ms/block (the low-noise signal --
            // tok/s is quantised by a whole generation step here):
            //     KT 0 (untiled) 1.951    KT 2 -> 1.784    KT 4 -> 1.853    KT 8 -> 1.880
            // Unimodal at 2. Wider tiles shorten the carried chain further but hold KT keys' worth
            // of V and KT*ROWS partial dots live, and at ROWS=8 that costs more occupancy than the
            // chain buys back. 0 keeps the one-key-at-a-time kernel for an A/B.
            static const int kt_env = []{
                const char* e = getenv("SPARKINFER_DFLASH_ATTN_KT");
                int v = e ? atoi(e) : 2;
                return (v == 0 || v == 2 || v == 4 || v == 8) ? v : 2;
            }();
#define SI_DF_ATTN_ROWS(R)                                                                    \
            do {                                                                                  \
                ...
                if (kt_env == 2)                                                                  \
                    ...
                    k_attn_rows_tile_hd128<(R), NWARPS, 4><<<g, NWARPS * 32, smem, stream>>>(     \
                        (const bf16*)q, (const bf16*)k, (const bf16*)v, fa_m, fa_l, fa_acc,       \
                        q_len, kv_lo, kv_len, n_q, n_kv, q_pos0, k_pos0, window, causal, scale,   \
                        n_splits);                                                                \
                else if (kt_env == 8)                                                             \
                    k_attn_rows_tile_hd128<(R), NWARPS, 8><<<g, NWARPS * 32, smem, stream>>>(     \
                        (const bf16*)q, (const bf16*)k, (const bf16*)v, fa_m, fa_l, fa_acc,       \
                        q_len, kv_lo, kv_len, n_q, n_kv, q_pos0, k_pos0, window, causal, scale,   \
                        n_splits);                                                                \
                else                                                                              \
