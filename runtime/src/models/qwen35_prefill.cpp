    ...
    if (!moe && !getenv("SPARKINFER_PREFILL_FFN_CHUNK")) {
        ...
        if (cudaMemGetInfo(&fb, &tb) == cudaSuccess) {
            ...
            while ((FC >> 1) >= kMinFfnChunk &&
                ...
                FC >>= 1;
            // At 32k on the hybrid Qwen3.8 stack, letting the dense FFN consume all recovered
            // capture memory leaves the 48-layer chunked GDN scan with pathological slices.
            // 3072 is the measured balance point: larger allocations cross scan-workspace
            // cliffs, while smaller chunks add GEMM launch overhead. This only limits scratch;
            // the per-token FFN arithmetic and output order are unchanged.
            if (c.hybrid && N >= 32768 && FC > 3072) FC = 3072;
            if (FC != fc_before)
