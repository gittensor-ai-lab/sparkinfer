    ...
    for (int L = 0; L < c.n_layers; L++) {
        ...
        if (w.linear_attn) {
            ...
            if (c.rope_dim > 0 && c.rope_dim < c.head_dim) {
                kernels::launch_rope_kv_append_partial(s.q, s.k, s.v, kpool, vpool, btable, s.d_pos, 1,
                                                       c.n_q_heads, c.n_kv_heads, c.head_dim, c.rope_dim,
                                                       c.rope_theta, s.kv->block_size(),
            ...
            proj_from(s.attn, w.wo, w.wo_type, s.ao, H, s.qdim);
        }
