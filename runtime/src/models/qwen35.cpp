...
std::vector<int> Qwen35Model::dflash_generate(const std::vector<int>& prompt, int max_new,
    ...
    // #878's narrowing was correct for the cost curve it measured and is not correct for this
    // one. Its case was that "each extra proposal is another row in the draft block... and another
    // row of the draft's own attention over the same KV", i.e. an extra verify row cost close to a
    // full extra pass over the model's weights, so proposals the target rejected were pure loss.
    // The NVFP4 projections are dp4a and row-batched now (this PR, on top of #904's dense FFN), so
    // an extra verify row costs the activation work alone and the weight stream is read once for
    // all rows. That flips the trade back.
    //
    // Measured at ctx=4096, same binary, DSPARK_TPS / MEAN_ACCEPT, against main's 106.70 / 1.5542:
    //
    //     depth   on main            with this PR
    //       1     106.70 / 1.5542    109.31 / 1.5542
    //       2     102.58 / 1.7333    110.95 / 1.7333
    //       3       -                118.41 / 1.9118   <- shipped
    //       4       -                 80.26 / 1.9403
    //
    // Depth 2 REGRESSES main by 3.9% and gains here: the narrowing and the un-narrowing are the
    // same measurement taken either side of the row-batching, which is why this is a re-tune and
    // not a revert. Depth 4 is the cliff #869 documented -- the draft's active block width rounds
    // up over {4, 8, 16}, so depth 4 runs an 8-wide draft block for 0.03 more accepted tokens.
    // Tied to the dp4a switch so the two move together: with the projections back on the float
    // path the old cost curve returns and so does depth 1.
    const bool nv_dp4a = kernels::qwen38_nvfp4_dp4a() && nvfp4_dp4a_proj();
    const int kProposalDepth = kProposalDepthEnv > 0 ? kProposalDepthEnv
                             : ((n + max_new) >= kDeepMinSeq ? 7
                                : (n >= kNarrowMinSeq ? (nv_dp4a ? 3 : 1) : 3));
